...
PVOID getAddress::GetProcAddrFromKernelPdb(wchar_t* zPdbName, wchar_t* szApiName)
{
...
{
	if (LoadDataFromPdb(L"res/ntkrnlmp.pdb"))
	{
		curBase = krnlBase;
		FindPublicAll(pSymbol);
	}

	if (LoadDataFromPdb(L"res/win32k.pdb"))
	{
		curBase = win32kBase;
		FindPublicAll(pSymbol);
	}
